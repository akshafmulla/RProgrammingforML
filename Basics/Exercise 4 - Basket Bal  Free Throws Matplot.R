...
freethrowattempts
colnames(freethrowattempts) <- c(Seasons)
rownames(freethrowattempts) <- c(Players)
#Solution

myplot <- function(z){
  matplot(t(z),type = "b",pch = 15:18, col = c(1:4,6))
  legend("bottomleft",inset=0.01,pch = 15:18, col = c(1:4,6),legend=Players,horiz = F)
}

#Free throw attempts per game
FTAPG <- freethrowattempts/Games
FTAPG
myplot(FTAPG)

#Accuracy of free throws
accuracyofft <- (freethrows/freethrowattempts)*100
accuracyofft
myplot(accuracyofft)

#player style excluding freethrows
