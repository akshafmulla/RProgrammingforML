...
RSC.25.medianage <- RSC.25[RSC.25$age <= median(RSC.25$age, 
                                                na.rm=T),]
...
aggregate(RSC[,c(8,10,11)], by=list(RSC$branch),
          FUN=sd) #FUN	is a function to compute the summary statistics which can be applied to all data subsets such as mean, median, std deviation etc.
